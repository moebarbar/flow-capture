...
import { 
  MessageSquare, Send, Reply, Check, X, 
  MoreVertical, Trash2 
} from "lucide-react";
import {
  DropdownMenu,
...
const ReplyItem = memo(function ReplyItem({ reply }: { reply: StepComment }) {
...
const CommentThread = memo(function CommentThread({
  ...
  const handleReplyContentChange = useCallback(
    (e: ChangeEvent<HTMLTextAreaElement>) => setReplyContent(e.target.value),
    [setReplyContent]
  );
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              {comment.isEditProposal && (
                <Badge variant="outline" className="text-xs">Edit Proposal</Badge>
              )}
              <span className="text-xs text-muted-foreground">{formattedTime}</span>
            </div>
            <DropdownMenu>
...
export const StepComments = memo(function StepComments({ stepId, guideId }: StepCommentsProps) {
  const { user } = useAuth();
  ...
  }, [replyContent, replyingTo, addCommentMutation]);

  const handleReply = useCallback((id: number) => setReplyingTo(id), []);
  const handleDelete = useCallback((id: number) => deleteCommentMutation.mutate(id), [deleteCommentMutation]);
  const handleCancelReply = useCallback(() => {
    setReplyingTo(null);
    setReplyContent("");
  }, []);
  const handleNewCommentChange = useCallback(
    (e: ChangeEvent<HTMLTextAreaElement>) => setNewComment(e.target.value),
    []
  );
  ...
  const getReplies = useCallback(
    (parentId: number) => comments.filter(c => c.parentId === parentId),
    [comments]
  );
